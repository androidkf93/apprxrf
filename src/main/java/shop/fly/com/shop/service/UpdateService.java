...
//		GoalApplication.mApp.setUpdateFlag(true);
		...
		// 设置任务栏中下载进程显示的views
		views = new RemoteViews(getPackageName(), R.layout.update_view);
			...
			public void run() {
				try {
					...
					OkHttpClient client = new OkHttpClient();
						...
						long lenght = responseBody.contentLength();
					InputStream is = response.body().byteStream();
						if (is != null) {
							...
							while ((read = bis.read(buffer)) != -1 && !cancelUpdate) {
								...
								count += read;
								precent = (int) (((double) count / lenght) * 100);
...
//					GoalApplication.mApp.setUpdateFlag(false);
